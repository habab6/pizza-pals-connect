...
import { Plus, Eye, Clock, CheckCircle, Truck, UserCheck, Settings, CreditCard, Trash2, Database, Receipt, Key, Edit3 } from "lucide-react";
...
import GestionCategories from "@/components/gestion/GestionCategories";
...
const CaissierDashboard = () => {
  ...
  const [showGestionCategories, setShowGestionCategories] = useState(false);
  ...
  const getGlobalStatus = (commande: any) => {
    ...
    // RÈGLE PRIORITAIRE : Si le statut global est "termine", on le retourne toujours
    // Cela évite que les commandes terminées repassent à un autre statut
    if (statutGlobal === 'termine') {
      return 'termine';
    }

    // Pour les livraisons, utiliser le statut global une fois en livraison/livré
    if (commande.type_commande === 'livraison' && 
        ['en_livraison', 'livre'].includes(statutGlobal)) {
      return statutGlobal;
    }
    ...
    // Commande d'un seul commerce - mais toujours respecter le statut global "termine"
    if (hasDolce && !hasLSF) {
      return statutDolce === 'termine' ? 'termine' : statutDolce;
    }
    if (hasLSF && !hasDolce) {
      return statutLSF === 'termine' ? 'termine' : statutLSF;
    }
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => setShowGestionCategories(true)}>
          ...
          <Dialog open={showGestionCategories} onOpenChange={setShowGestionCategories}>
            <DialogContent className="max-w-[95vw] md:max-w-4xl h-[90vh] max-h-[95vh] overflow-hidden flex flex-col">
              <GestionCategories onClose={() => setShowGestionCategories(false)} />
            </DialogContent>
          </Dialog>
